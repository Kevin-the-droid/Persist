...
from backend.services.tools import execute_tool, get_enabled_tools, build_tool_manifest, parse_minimax_tool_calls, format_tool_result_message, get_active_todos, format_todos_for_prompt
...
async def _chat_stream_internal(
    ...
    # Active todos — shown every turn so Kevin always sees current working state.
    # Lives in user message (not system) so KV cache stays stable across turns.
    active_todos = get_active_todos(agent.id, db)
    todo_block = format_todos_for_prompt(active_todos)
    if todo_block:
        user_msg_parts.append(todo_block)
