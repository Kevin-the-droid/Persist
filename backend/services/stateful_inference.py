...
class StatefulInferenceEngine:
    ...
    def __init__(self):
        ...
        # ALSO: MLX 0.30+ made streams thread-local. mlx_lm.generate creates its
        # `generation_stream` global on whatever thread first imports the
        # module; that has to be THIS executor thread (where generation will
        # later run), not the main asyncio thread. So we defer the mlx_lm
        # imports until the first executor task runs.
    ...
    def _import_mlx_on_executor(self):
        """Bind mlx_lm globals on the executor thread.

        MLX 0.30+ made streams thread-local, but `mlx_lm.generate` creates its
        `generation_stream` module global at import time on the FIRST thread to
        import it. Other backend services (reranker, embedding server) import
        mlx_lm at module top level during FastAPI boot, so by the time we get
        here that stream is bound to the main thread — which crashes when
        generation actually runs on this executor thread.

        Fix: rebind `mlx_lm.generate.generation_stream` to a fresh stream
        created on THIS thread. The old main-thread stream becomes orphaned,
        which is fine — nothing else uses it for the inference path.
        """
        ...
        # inside mlx_lm find a stream that belongs to THIS thread.
        _gen_module.generation_stream = _mx.new_stream(_mx.default_device())

        self._mlx_imported = True
        print(
            f"[StatefulInference] mlx_lm imported on inference thread; "
            f"rebound generation_stream to {_gen_module.generation_stream}"
        )
    ...
    async def ensure_model(self, model_path: str, adapter_path: Optional[str] = None):
        ...
        def _load():
            # First task on this thread — make sure mlx_lm is imported here
            # (so its module-level generation_stream is bound to THIS thread).
